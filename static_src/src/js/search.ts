...
function renderResults(
	results: Array<EnrichedDocumentSearchResultSetUnit<Posts>>,
) {
	...
	if (searchResults) {
		...
		arr.forEach((result) => {
			...
			const bodyStartPos =
				matchPos - BODY_LENGTH / 2 > 0 ? matchPos - BODY_LENGTH / 2 : 0;
			resultBody.innerHTML =
				result.doc.body?.substr(bodyStartPos, BODY_LENGTH) ?? "";
